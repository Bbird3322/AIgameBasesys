...
- `LLMGameBaseLauncher.exe` の生成に Visual Studio の C/C++ コンパイラは不要です
- EXE ビルドは PowerShell と `ps2exe`、内部で使われる .NET 系コンパイラで完結します
- オフライン環境で EXE ビルドする場合は、先に `ps2exe` が入っている必要があります
- `llama-server.exe` の実行側では、ビルドによって Microsoft Visual C++ 再頒布可能パッケージや CUDA runtime が必要になる場合があります
- GPU モードが動かない場合は、まず CUDA runtime と `cublas64_13.dll` の有無を確認してください
