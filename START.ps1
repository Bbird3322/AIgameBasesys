...
function Invoke-Launcher {
  ...
  $logDir = Join-Path (Get-LauncherRoot) "logs"
  ...
  if ($launchArguments.Count -gt 0) {
    $proc = Start-Process -FilePath $targetPath -ArgumentList $launchArguments -PassThru -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog
  } else {
